package com.example.common_module.network.interceptor.log

import android.text.TextUtils
import com.example.common_module.ext.string.jsonFormat
import com.example.common_module.ext.string.xmlFormat
import com.example.common_module.log.LogUtil
import com.example.common_module.network.interceptor.log.LogInterceptor.Companion.isJson
import com.example.common_module.network.interceptor.log.LogInterceptor.Companion.isXml
...
class DefaultFormatPrinter: FormatPrinter {
    ...
    override fun printJsonResponse(
        ...
        bodyString =
            when {
                isJson(contentType) -> bodyString!!.jsonFormat()
                isXml(
                    contentType
                ) -> bodyString!!.xmlFormat()
                else -> bodyString
            }
        val responseBody =
            LINE_SEPARATOR + BODY_TAG + LINE_SEPARATOR + bodyString
